<!-- Loaded by the backend agent in both planning modes. -->

## Verification and recovery

After a coherent backend change, run the relevant commands from
`input/tech/stack.md`: lint, type checking, build, and tests when they apply.
Choose focused checks first, then broaden verification in proportion to the
change and its risk.

When a check fails:

- inspect the actual error and enough surrounding code to identify the cause;
- fix the cause rather than weakening assertions, skipping checks, or hiding a
  type error;
