...
|-------|--------|----------|
| Feature/spec slug (`$ARGUMENTS`) | `/planr:plan` orchestrator | Yes |
...
- Resolve bare relative paths against the **project root** (working directory), NOT against `${CLAUDE_PLUGIN_ROOT}/references/pipeline`
...
```
0. Receive spec slug from /planr:plan as $ARGUMENTS (the {slug} in SPEC-NNN-{slug})
